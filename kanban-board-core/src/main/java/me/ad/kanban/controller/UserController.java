...
public class UserController {
    ...
    @Autowired
    public UserController(CustomMessageProperties message, UserService userService, AuthorizationService authorizationService) {
    ...
    public void deleteUserById(@PathVariable("id") String id, Authentication auth) {
        authorizationService.ensureAuthUserIsConcernedUser(auth, id);
        userService.deleteUserById(id);
    }

    @PostMapping(path = "/remove", consumes = {MediaType.APPLICATION_JSON_VALUE})
    public void deleteUserByIdList(@RequestBody List<String> idList, Authentication auth) {
        idList.forEach((id) -> authorizationService.ensureAuthUserIsConcernedUser(auth, id));
